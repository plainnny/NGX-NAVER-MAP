...
import * as mapTypes from '../services/maps-types';
import {MarkerManager} from '../services/managers/marker-manager';

import {AnmInfoWindow} from './info-window';
import {ImageIcon} from "../services/maps-types";

let markerId = 0;

/**
 * AnmMarker renders a map marker inside a {@link AnmMap}.
 *
 * ### Example
 * ```typescript
...
export class AnmMarker implements OnDestroy, OnChanges, AfterContentInit {
  /**
   ...
   * @internal
   */
  @ContentChildren(AnmInfoWindow) infoWindow: QueryList<AnmInfoWindow> = new QueryList<AnmInfoWindow>();
  ...
  ngOnChanges(changes: {[key: string]: SimpleChange}) {
    ...
    if (changes['latitude'] || changes['longitude']) {
      if(!!this.latitude && !!this.longitude) this._markerManager.updateMarkerPosition(this);
    }
    if (changes['title']) {
  ...
  /** @internal */
  toString(): string { return 'AnmMarker-' + this._id.toString(); }
