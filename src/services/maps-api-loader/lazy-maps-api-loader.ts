...
export enum MapsScriptProtocol {
  HTTP = 1,
  HTTPS = 2,
  AUTO = 3
}
...
export class LazyMapsAPILoader extends MapsAPILoader {
  ...
  load(): Promise<void> {
    ...
    script.src = this._getScriptSrc(null);

    this._documentRef.getNativeDocument().body.appendChild(script);
    this._scriptLoadingPromise = new Promise<void>((resolve: Function, reject: Function) => {
      (<any>this._windowRef.getNativeWindow()).initMap = function () {
        // alert('callback~~');
        resolve();
      };
      // script.onload = () => {
      //   (<any>this._windowRef.getNativeWindow()).initMap();
      // };
  ...
  private _getScriptSrc(callbackName: string): string {
    const protocolType: MapsScriptProtocol =
        (this._config && this._config.protocol) || MapsScriptProtocol.HTTPS;
    let protocol: string;

    switch (protocolType) {
      case MapsScriptProtocol.AUTO:
        protocol = '';
        break;
      case MapsScriptProtocol.HTTP:
        protocol = 'http:';
        break;
      case MapsScriptProtocol.HTTPS:
        protocol = 'https:';
        break;
    }

    const hostAndPath: string = this._config.hostAndPath || 'openapi.map.naver.com/openapi/v3/maps.js';
    const queryParams: {[key: string]: string | Array<string>} = {
      clientId: this._config.apiKey,
      callback: 'initMap'
    };
    const params: string =
